...
import torch 
import numpy as np
#sys.path.append('/scratch/zhangzy/code/bias-correction/deep-learning-for-heavy-rainfall-v2.0.0/deep-learning-for-heavy-rainfall-04b2dd7/src/')
...
# def CSI(pred,target,threshold,cos):
#     func = pysteps.verification.interface.get_method('CSI', type='deterministic')
#     batch = pred.shape[0]
#     time_step = pred.shape[1]
#     lat = pred.shape[2]
#     csi = np.zeros([batch,time_step,lat])

#     for i in range(batch):
#         for j in range(time_step):
#             for k in range(lat):
#                 csi[i,j,k]=func(pred[i,j,k],target[i,j,k],thr=threshold)['CSI'] 
#                 # csi[i,j,k]=csi[i,j,k]*cos[i,k] if np.isnan(csi[i,j,k])==False else 0.0
#     csi = csi.mean(axis=2)
#     return csi
...
def ETS(pred,target,threshold):
    ...
    for i in range(batch):
        for j in range(time_step):
            ets[i,j]=func(pred[i,j],target[i,j],thr=threshold)['ETS']     
    return ets
...
def RMSE(pred,target):
    ...
    batch = pred.shape[0]
    time_step = pred.shape[1]
    rmse = np.zeros([batch,time_step])
    for i in range(batch):
        for j in range(time_step):
            rmse[i,j]=func(pred[i,j],target[i,j])['RMSE']
    return rmse
...
def accuracy(pred,target):
    Nfr = np.sum(abs(pred-target)<2.0)
