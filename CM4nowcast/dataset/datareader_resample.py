...
DEFAULT_CATALOG = '/root/autodl-tmp/SEVIR/CATALOG.csv'
...
def get_sevir_loader_spilt(batch_size=3,
                     x_img_types=['vil'],
                     y_img_types=None,
                     catalog=DEFAULT_CATALOG,
                     start_date=None,
    ...
    #train_weights = get_weights(train_dataset_split)
    #val_weights = get_weights(val_dataset_split)
    #train_sampler = WeightedRandomSampler(train_weights, len(train_dataset_split)*3, replacement=True)
    #val_sampler = WeightedRandomSampler(val_weights, len(val_dataset_split)*3, replacement=True)
    return DataLoader(train_dataset_split,batch_size=batch_size,shuffle=shuffle,pin_memory=pin_memory,num_workers=num_workers, drop_last=True, sampler=train_sampler), DataLoader(val_dataset_split,batch_size=batch_size,shuffle=shuffle,pin_memory=pin_memory,num_workers=num_workers, drop_last=True, sampler=val_sampler)
...
class SEVIR(Dataset):
    ...
    def __get_weights__(self):
        ...
        for item in self._samples:
            if (self._samples[item].urcrnrlon.values + self._samples[item].llcrnrlon.values)/2 <= -110:
                weights.append(5.0)
                weights.append(5.0)
                weights.append(5.0)
            else:
