...
class InterscaleTrans(nn.Module):
    def __init__(self, dim=256,channels = [48,96], num=1):
...
class MultiscaleTrans(nn.Module):
    def __init__(self, size = 384, channels = [1024,512]):
        super(MultiscaleTrans, self).__init__()
        self.block_layer = [2, 2]
        self.size = [size//12, size//24]
        self.channels = channels
        self.R = 2
        stage1 = []
    ...
    def forward(self, x):
        ...
        feature1_in = self.squeeze_final(feature1_in)
        feature1_trans = self.stage2(feature1_in)

        return [feature0_trans, feature1_trans]
