...
class CuboidTransformerModel(nn.Module):
    ...
    def get_initial_encoder_final_decoder(
        ...
        self.scale_encorder1 = MultiscaleTrans(channels = [128,256])
        self.scale_encorder2 = InterscaleTrans(dim=256, channels = [128,256])
        self.fusionlayers = nn.ModuleList()
        self.fusion_list = [128, 256]
        for i in range(2):
    ...
    def forward(self, x, verbose=False):
        ...
        mem_l[0]= mem_l[0].view(b*t,h,w,c).permute(0,3,1,2).contiguous()
        b, t, h, w, c = mem_l[1].shape
        b2, t2, h2, w2, c2 = mem_l[1].shape
        mem_l[1]= mem_l[1].view(b*t,h,w,c).permute(0,3,1,2).contiguous()
