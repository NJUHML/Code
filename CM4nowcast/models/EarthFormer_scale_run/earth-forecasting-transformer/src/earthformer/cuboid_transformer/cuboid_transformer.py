...
from Scale_Module_For_Use_2 import MultiscaleTrans, InterscaleTrans
...
class CuboidTransformerModel(nn.Module):
    ...
    def get_initial_encoder_final_decoder(
        ...
        self.scale_encorder1 = MultiscaleTrans(channels = [1536,3072])
        self.scale_encorder2 = InterscaleTrans(dim=256, channels = [1536,3072])
        self.fusionlayers = nn.ModuleList()
        self.fusion_list = [1536, 3072]
        for i in range(2):
    ...
    def forward(self, x, verbose=False):
        ...
        #print(mem_l[1].shape)
        #print(mem_l[2].shape)
        b, t, h, w, c = mem_l[0].shape
        b1, t1, h1, w1, c1 = mem_l[0].shape
        mem_l[0]= mem_l[0].permute(0,1,4,2,3).contiguous().view(b,t*c,h,w)
        b, t, h, w, c = mem_l[1].shape
        b2, t2, h2, w2, c2 = mem_l[1].shape
        mem_l[1]= mem_l[1].permute(0,1,4,2,3).contiguous().view(b,t*c,h,w)
        feature_trans = self.scale_encorder1(mem_l)
        ...
        mem_l[0] = self.fusionlayers[0](torch.cat((feature_trans[0], mem_l[0]), dim=1))
        mem_l[1] = self.fusionlayers[1](torch.cat((feature_trans[1], mem_l[1]), dim=1))

        
        mem_l[0]= mem_l[0].view(b1,t1,c1,h1,w1).permute(0,1,3,4,2).contiguous()
        mem_l[1]= mem_l[1].view(b2,t2,c2,h2,w2).permute(0,1,3,4,2).contiguous()
        #print(mem_l[1].shape)
        #print(mem_l[0].shape)
        if verbose:
