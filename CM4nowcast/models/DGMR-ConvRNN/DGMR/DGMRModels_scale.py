...
sys.path.append('/root/code/CM4nowcast/models/MEFM')
from Scale_Module_For_Use import MultiscaleTrans, InterscaleTrans
sys.path.append('/root/code/CM4nowcast/models/DGMR-ConvRNN')
...
class DGMRGenerator(nn.Module):
    def __init__(
        ...
        self.conv0 = nn.Sequential(
                nn.utils.parametrizations.spectral_norm(nn.Conv2d(int(96/4*6), 48, 3, 1, 1)),
                nn.ReLU(inplace=True)
                )
        self.conv1 = nn.Sequential(
                nn.utils.parametrizations.spectral_norm(nn.Conv2d(int(192/4*6), 96, 3, 1, 1)),
                nn.ReLU(inplace=True)
                )
        self.conv2 = nn.Sequential(
                nn.utils.parametrizations.spectral_norm(nn.Conv2d(int(384/4*6), 192, 3, 1, 1)),
                nn.ReLU(inplace=True)
                )
        self.conv3 = nn.Sequential(
                nn.utils.parametrizations.spectral_norm(nn.Conv2d(int(768/4*6), 384, 3, 1, 1)),
                nn.ReLU(inplace=True)
                ) 
        ...
        self.scale_encorder1 = MultiscaleTrans(size=size, channels = [48,96, 192, 384])
        self.scale_encorder2 = InterscaleTrans(dim=256, channels = [48,96, 192, 384])
        self.fusionlayers = nn.ModuleList()
        self.fusion_list = [48, 96, 192, 384]
        for i in range(4):
            self.fusionlayers.append(
    ...
    def forward(self, x0, return_noise=False):
        ...
        feature_trans = self.scale_encorder1([x0, x1, x2, x3])
        #print(feature_trans[0].shape)
        #print(feature_trans[1].shape)
        #print(feature_trans[2].shape)
        #print(feature_trans[3].shape)
        feature_trans = self.scale_encorder2(feature_trans)
        x0 = self.fusionlayers[0](torch.cat((feature_trans[0], x0), dim=1))
        x1 = self.fusionlayers[1](torch.cat((feature_trans[1], x1), dim=1))
        x2 = self.fusionlayers[2](torch.cat((feature_trans[2], x2), dim=1))
        x3 = self.fusionlayers[3](torch.cat((feature_trans[3], x3), dim=1))
