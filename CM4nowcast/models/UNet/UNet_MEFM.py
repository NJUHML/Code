...
class ParallEncoder(nn.Module):
    def __init__(self, input_channel=6,size= 384):
        super(ParallEncoder, self).__init__()
        self.Encoder1 = UEncoder(input_channel = input_channel)
        self.Encoder2 = MultiscaleTrans(size=size)
        self.fusion_module = nn.ModuleList()
        self.num_module = 4
        self.channel_list = [128, 256, 512, 1024]
        self.fusion_list = [256, 512, 1024, 1024]
        self.inter_trans = InterscaleTrans(dim=256)
...
class MultiscaleTrans(nn.Module):
    def __init__(self, size = 384):
        super(MultiscaleTrans, self).__init__()
        self.block_layer = [2, 2, 2, 1]
        self.size = [size//4, size//8, size//16, size//32]
        self.channels = [256, 512, 1024, 1024]
        self.R = 4
        stage1 = []
        for _ in range(self.block_layer[0]):
            stage1.append(
                MultiscaleattenBlock(
...
class UNet_CM(nn.Module):
    def __init__(self, input_channel = 6, n_classes = 12,size = 384):
        super(UNet_CM, self).__init__()
        self.p_encoder = ParallEncoder(input_channel=input_channel,size = size//3)
