...
class Discriminator(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        num_spatial_frames = num_spatial_frames
        conv_type = conv_type
...
class Discriminator4IMERG(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_spatial_frames: int = 2,
        conv_type: str = "standard",
    ):
        super().__init__()
        input_channels = input_channels
        ...
        self.temporal_discriminator = SpatialDiscriminator(
            input_channels=input_channels, num_timesteps=num_spatial_frames, conv_type=conv_type
        )
...
class TemporalDiscriminator(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.downsample = torch.nn.AvgPool3d(kernel_size=(1, 3, 3), stride=(1, 3, 3))
...
class Discriminator1(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_spatial_frames: int = 12,
        conv_type: str = "standard",
    ):
        super().__init__()
        input_channels = input_channels
        num_spatial_frames = num_spatial_frames
        conv_type = conv_type
        self.spatial_discriminator = SpatialDiscriminator1(
            input_channels=input_channels, num_timesteps=num_spatial_frames, conv_type=conv_type
        )
        self.temporal_discriminator = TemporalDiscriminator1(
            input_channels=input_channels, conv_type=conv_type
        )
...
#import sys
#sys.path.append("/share/home/veiga2/yangss/pycode/nowcasting/UNet_pytorch/")
#from models.discriminator import weights_init_zeros, Discriminator_STMFA, Discriminator_STMFA1

class Discriminator_my(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_spatial_frames: int = 12,
        ...
        num_spatial_frames = num_spatial_frames
        conv_type = conv_type
        self.spatial_discriminator = Discriminator_STMFA((384, 384), 1, 13, 12, 64)
        self.spatial_discriminator.apply(weights_init_zeros)
        self.temporal_discriminator = TemporalDiscriminator1(
            input_channels=input_channels, conv_type=conv_type
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b,_,_,_ = x.shape
        spatial_loss = self.spatial_discriminator(x, b)
        x = x.unsqueeze(2)
        #spatial_loss = self.spatial_discriminator(x)
        temporal_loss = self.temporal_discriminator(x)
...
class Discriminator_my_1(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_spatial_frames: int = 12,
        ...
        num_spatial_frames = num_spatial_frames
        conv_type = conv_type
        self.spatial_discriminator = Discriminator_STMFA1((384, 384), 1, 13, 12, 64)
        self.spatial_discriminator.apply(weights_init_zeros)
        self.temporal_discriminator = TemporalDiscriminator1(
            input_channels=input_channels, conv_type=conv_type
        )
...
class Discriminator_my_2(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_spatial_frames: int = 12,
        ...
        self.spatial_discriminator = Discriminator_STMFA1((384, 384), 1, 13, 12, 64)
        self.spatial_discriminator.apply(weights_init_zeros)
        self.temporal_discriminator = TemporalDiscriminator2(
            input_channels=input_channels, conv_type=conv_type
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat([spatial_loss, temporal_loss], dim=0)


class Discriminator_my_3(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_spatial_frames: int = 12,
        ...
        self.spatial_discriminator = Discriminator_STMFA1((384, 384), 1, 13, 12, 64)
        self.spatial_discriminator.apply(weights_init_zeros)
        self.temporal_discriminator = TemporalDiscriminator3(
            input_channels=input_channels, conv_type=conv_type
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat([spatial_loss, spatial_loss, spatial_loss, temporal_loss], dim=0)




class TemporalDiscriminator1(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self, input_channels: int = 1, num_layers: int = 3, conv_type: str = "standard"):
        """
        Temporal Discriminator from the Skillful Nowcasting, see https://arxiv.org/pdf/2104.00954.pdf
        Args:
            input_channels: Number of channels per timestep
...
class TemporalDiscriminator2(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self, input_channels: int = 1, num_layers: int = 3, conv_type: str = "standard"):
        """
        Temporal Discriminator from the Skillful Nowcasting, see https://arxiv.org/pdf/2104.00954.pdf
        ...
        self.fc = spectral_norm(torch.nn.Linear(6 * 2 * internal_chn * input_channels, 1))
        self.relu = torch.nn.ReLU()
        self.bn = torch.nn.BatchNorm1d(2 * internal_chn * input_channels)
        self.sigmoid = torch.nn.Sigmoid()
...
class TemporalDiscriminator3(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self, input_channels: int = 1, num_layers: int = 3, conv_type: str = "standard"):
        """
        Temporal Discriminator from the Skillful Nowcasting, see https://arxiv.org/pdf/2104.00954.pdf
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for idx in range(x.size(1)):
            ...
            rep = self.bn(rep)
            rep = self.fc(rep)
            rep = self.sigmoid(rep)

            representations.append(rep)
        # The representations are summed together before the ReLU
        x = torch.stack(representations, dim=0)
        # Should be [Batch, N, 1]
        #x = torch.sum(x, dim=1)
...
class SpatialDiscriminator1(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        input_channels: int = 1,
        num_timesteps: int = 6,
...
if __name__ == "__main__":
    import torchinfo
    model = Discriminator()
    summary(model, input_size = (4,20,1,384,384), col_names = ["output_size", "kernel_size"])
